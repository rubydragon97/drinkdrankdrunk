...
//   // const alices = await prisma.users({ where: { name: 'Alice' } });
//   // const mary = alices[0];
//   // mary.email = "mary@alice.lol"
//   // await prisma.updateUser({ where: { id: mary.id }, data: { email: mary.email } });
//   // await prisma.deleteUser({ id: alices[0].id });
...
const Mutation = prismaObjectType({
  name: 'Mutation',
  definition(t) {
    t.prismaFields(['createUser', 'createDrink'])
